...
@Slf4j
@Component
public class BZNWarehouseAction extends AbstractWarehouse {
	...
	public void batchSaveWarehouseMergeExcel(List<BatchAddWarehouseWorkRequest> requestData) throws ExceptionPack {
		...
		try {
			// === 性能优化1：批量预处理数据，减少循环次数 ===
			Set<Long> allProductPartCodes = new HashSet<>();
			Set<Long> allPositionCodes = new HashSet<>();
				...
				if (positionList != null) {
					for (WarehousePositionDetailModel model : positionList) {
						if (model != null && model.getPositionCode() != null) {
							...
							allPositionCodes.add(model.getPositionCode());
						}
					}
				}
			...
			requestData.forEach(request -> {
				if (request.getWarehousePositionList() != null && !request.getWarehousePositionList().isEmpty()) {
					request.getWarehousePositionList().forEach(detail -> {
						...
						if (detail.getProductPartCode() != null) {
							allProductPartCodes.add(detail.getProductPartCode());
						}
					});
				}
			});
			...
			// === 性能优化2：批量查询，避免N+1问题 ===
			// 库存code数据准备 - 批量查询所有产品
			List<Long> productPartCodeList = new ArrayList<>(allProductPartCodes);
			...
			// 批量查询所有仓位库存数据，避免在循环中重复查询
			List<Long> allPositionCodesList = new ArrayList<>(allPositionCodes);
			Map<String, PositionStockModel> globalPositionStockModels = positionStockMapper.getProductByPositionList(
					allPositionCodesList,
					productPartCodeList,
					enterpriseCode,
			...
			// === 性能优化3：收集批量操作，减少数据库交互次数 ===
			List<PositionStockModel> globalPositionStockModelAddList = new ArrayList<>();
			List<PositionStockBatchModel> globalBatchAddList = new ArrayList<>();
			List<PositionStockBatchModel> globalBatchUpdateList = new ArrayList<>();
			List<PositionOperationModel> globalStorePositionOperationModelList = new ArrayList<>();
			List<PositionOperationModel> globalOutPositionOperationModelList = new ArrayList<>();
			...
			for (BatchAddWarehouseWorkRequest requestModel : requestData) {
				...
				for (WarehousePositionDetailModel positionDetailModel : requestModel.getWarehousePositionList()) {
					...
					// === 性能优化4：使用全局缓存的仓位信息，避免重复查询 ===
					PositionStockModel positionStockModel = globalPositionStockModels.get(positionAndProductCode);
				...
				// 收集到全局批量操作列表中
				globalPositionStockModelAddList.addAll(positionStockModelAddList);
				globalBatchAddList.addAll(batchAddList);
				globalBatchUpdateList.addAll(batchUpdateList);
				globalStorePositionOperationModelList.addAll(storePositionOperationModelList);
				globalOutPositionOperationModelList.addAll(outPositionOperationModelList);
			...
			// === 性能优化5：批量执行所有数据库操作 ===
			// 批量更新库存批次数据
			if (ObjectUtil.isNotEmpty(globalBatchUpdateList)) {
				List<PositionStockBatchModel> uniqueBatchUpdateList = globalBatchUpdateList.stream()
			...
			// 批量出库操作记录
			if (ObjectUtil.isNotEmpty(globalOutPositionOperationModelList)) {
				log.info("批量执行出库操作记录，数量: {}", globalOutPositionOperationModelList.size());
				positionOperationMapper.insertBatch(globalOutPositionOperationModelList, positionOperationTableName);
			}

			// 批量入库操作记录
			if (ObjectUtil.isNotEmpty(globalStorePositionOperationModelList)) {
				log.info("批量执行入库操作记录，数量: {}", globalStorePositionOperationModelList.size());
				positionOperationMapper.insertBatch(globalStorePositionOperationModelList, positionOperationTableName);
			}

			// 批量新增仓库仓位数据
			if (ObjectUtil.isNotEmpty(globalPositionStockModelAddList)) {
				log.info("批量新增仓库仓位数据，数量: {}", globalPositionStockModelAddList.size());
				positionStockMapper.insertBatch(globalPositionStockModelAddList, positionStockTableName);
			}
			
			// 批量新增批次数据
			if (ObjectUtil.isNotEmpty(globalBatchAddList)) {
				log.info("批量新增批次数据，数量: {}", globalBatchAddList.size());
				try (AutoCloseable ignored = RequestTableHelper::clearBatchTableName) {
					RequestTableHelper.setBatchTableName(positionStockBatchTableName);
					positionStockBatchService.saveBatch(globalBatchAddList);
				} catch (Exception e) {
					throw new RuntimeException(e);
				}
			}
			...
			for (BatchAddWarehouseWorkRequest requestModel : requestData) {
				...
				for (WarehousePositionDetailModel positionDetailModel : requestModel.getWarehousePositionList()) {
					Long positionCode = positionDetailModel.getPositionCode();
					if (positionCode != null) {
						BigDecimal batchNum = ObjectUtil.defaultIfNull(positionDetailModel.getPositionBatchNum(), BigDecimal.ZERO);
						positionCodeToBatchNumSum.merge(positionCode, batchNum, BigDecimal::add);
					}
				}
