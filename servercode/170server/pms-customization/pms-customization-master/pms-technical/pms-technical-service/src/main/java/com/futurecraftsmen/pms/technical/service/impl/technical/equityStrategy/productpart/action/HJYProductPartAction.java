...
@Slf4j
@Component
public class HJYProductPartAction extends AbstractProductPart {
	...
	public RpcPagingDTO<ProductPartRpcDTO> getPageList(ProductPartPageRequest requestData) throws ExceptionPack {
		try {
			...
			stopWatch.stop();
			log.info("\r\n开始执行产品零件列表信息-请求执行耗时：{}", stopWatch.prettyPrint(TimeUnit.MILLISECONDS));
			return new RpcPagingDTO<>(data, pageList.getTotal());
		} catch (Exception e) {
			throw new ExceptionPack(e, ExceptionMsg.builder("query HJYProductPartAction.getPageList failed").build());
		}
	}
	...
	public void batchAddProductPart(List<?> requestData) throws ExceptionPack {
		...
		try {
			...
			for (ProductPartBatchAddRpcRequest addRpcRequest : requestListData) {
				ProductPartAddRpcRequest partAddRpcRequest = Convert.convert(ProductPartAddRpcRequest.class, addRpcRequest);
				// 如果是产品：产品名称可以重复、产品型号不可以重复
				// 如果是零件：零件名称+型号，唯一
				productPartCommonService.validateProductOrPartAdd(partAddRpcRequest, productPartTableName);
			}
			// 获取统一数据 图纸号数据
			...
			for (ProductPartBatchAddRpcRequest addRpcRequest : requestListData) {
				...
				//List<String> modelListData =
				//		ExtractUtil.streamMapToList(String::valueOf, addRpcRequest.getPartInfoBatchAddModels(),
				//				ProductPartBatchAddRpcRequest.PartInfoBatchAddModel::getModelSpecification);
				//Map<String, ProductPartModel> partToModelMap = unifiedDataService.preparePartModelData(modelListData,
			...
			throw new ExceptionPack(e,
					ExceptionMsg.builder("query IProductPartProcedureService.getPageList failed").build());
		}
	}
